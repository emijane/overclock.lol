...
| [overclock/lib/supabase/auth-identity.ts](/c:/Users/misa/Documents/GitHub/overclock.lol/overclock/lib/supabase/auth-identity.ts:1) | Shared auth-cookie and optional auth-subject helpers | Server-only support helper | Cookie heuristics are useful for optional public-route fast paths, but should not be treated as the sole authorization source of truth. |

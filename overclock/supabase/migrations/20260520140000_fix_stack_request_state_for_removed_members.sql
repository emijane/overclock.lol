-- fix: get_lfg_feed_page_dto returned stackRequestState = 'accepted' for removed members.
--
-- root cause: the stackRequestState CASE fell through to
--   coalesce(vsr.status, 'none')
-- where vsr is viewer_stack_requests. that CTE holds the most recent stack_requests row
-- for the viewer with no status filter. when a member is removed, stack_members.removed_at
-- is set (correctly excluded by viewer_stack_memberships), but stack_requests.status is
-- still 'accepted'. the fallback coalesce therefore returned 'accepted' on every fresh
-- page load, causing the removed member to always see "Leave Stack".
--
-- fix: only propagate 'pending' and 'declined' from stack_requests in the fallback.
-- 'accepted' is only valid when confirmed by stack_members (removed_at IS NULL) via vsm.
-- if the viewer has an accepted request but is not in stack_members, they were removed:
-- return 'none' so the card shows the correct non-member state.
      ...
      jsonb_build_object(
        ...
        'stackRequestState',
          case
            ...
            when vsr.status = 'pending' or vsr.status = 'declined' then vsr.status
            else 'none'
          end
      )

-- Optimize get_lfg_feed_page_dto: replace per-row correlated EXISTS subqueries
-- with pre-loaded CTEs joined once for the full result set.
--
-- Before: up to 4 correlated subqueries per post (profile_connections EXISTS,
--         play_invites EXISTS, stack_members EXISTS, stack_requests subselect),
--         plus 2 separate scalar subselects hitting competitive_profiles for
--         main_role and platform.
-- After:  4 CTEs that each scan once for all 30 posts; competitive_profiles
--         fetched in a single SELECT INTO.
...
begin
  ...
  -- One scan of profile_connections for the viewer against all feed post authors.
  -- Replaces a per-row EXISTS that ran once per post.
  ...
  -- One scan of play_invites for the viewer's pending outgoing invites to feed posts.
  -- Replaces a per-row EXISTS that ran once per post.
  viewer_invites as (
  ...
  -- One scan of stack_members for the viewer's active membership in feed posts.
  -- Replaces a per-row EXISTS that ran once per stacks post.
  ...
  -- One scan of stack_requests for the viewer's most recent request status per post.
  -- Replaces a per-row subselect that ran once per stacks post.
  ...
  if v_viewer_profile_id is not null then
    -- Single lookup replacing two separate scalar subselects on competitive_profiles.

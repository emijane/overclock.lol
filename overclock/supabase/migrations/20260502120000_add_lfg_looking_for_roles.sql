alter table public.lfg_posts
  add column if not exists looking_for_roles text[];

update public.lfg_posts
set looking_for_roles = array['tank', 'dps', 'support']::text[]
where looking_for_roles is null;

alter table public.lfg_posts
  alter column looking_for_roles set default array['tank', 'dps', 'support']::text[],
  alter column looking_for_roles set not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'lfg_posts_looking_for_roles_check'
      and conrelid = 'public.lfg_posts'::regclass
  ) then
    alter table public.lfg_posts
      add constraint lfg_posts_looking_for_roles_check
      check (

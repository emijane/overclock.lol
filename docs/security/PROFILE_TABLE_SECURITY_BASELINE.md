...
older core profile tables whose original bootstrap migrations are not present in
this repo.
...
column, constraint, or index on those older tables. If the repo later needs a
full schema bootstrap for local-from-scratch database reconstruction, that
should be added as a separate canonical schema snapshot or baseline DDL pass.
